const Controller = {
    ...
    onMouse: function (e) {
        if (e.type == "mousedown") {
            ...
            if (
                local_pos[1] > World.current_room.range[2] &&
                local_pos[1] < World.current_room.range[3]
            ) {
            ...
            World.current_room.exits.forEach((exit) => {
                if (exit.active) {
                    if (
                        local_pos[0] > exit.range[0] &&
                        local_pos[0] < exit.range[1] &&
                        local_pos[1] > EXIT_SIGN_Y &&
                        local_pos[1] < EXIT_SIGN_Y + EXIT_SIGN_HEIGHT
                    ) {
